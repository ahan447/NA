	...
	ActionListener handler = new ActionListener() {
		...
		public void actionPerformed(ActionEvent e) {
			...
			//파일의 전체 크기 확인
				...
				//내용 읽기
				fis.read(b);			
				//읽은 내용을 문자열로 만들기(UTF-8 <> MS944(맥 기본미리보기로는 ms949 지원안함)
				...
				display.setText(result);
					...
					display.setText("읽을 파일이 존재하지 않음");
				}
				...
				try(FileOutputStream fos = new FileOutputStream(filePath, true)){
					...
					//(UTF-8 <> MS944(맥 기본미리보기로는 ms949 지원안함)
				...
				try(PrintStream ps = new PrintStream(
					...
					String msg = display.getText();
							//byte [] b = msg.getBytes();
							//write는 바이트로 바꾸고 넣어줘야함
							//ps.write(b);
							//print는 바이트로 바꿀 필요가 없음
							ps.print(msg);
							
							ps.flush();
							
				}catch (Exception e1) {
					display.setText("파일 경로 재확인");
				}
			}
		}
	};
		
	//2개의 버튼을 액션 이벤트를 handler 하나로 처리 - 이벤트 라우팅
	//유사한 역할을 하는 객체가 여러개 있을 때
	//객체 별로 이벤트 처리를 하게 되면 코드가 중복되서
	//유지보수를 어렵게 할 수 있음
