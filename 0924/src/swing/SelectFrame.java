...
public class SelectFrame extends JFrame {
	...
	//ComboBox 와 JList에 데이터를 출력하기 위한 배열
	String [] ar;
	String [] br;
	...
	public SelectFrame() {
		...
		//btn을 누르면 oop에서 선택한 항목과 language에서 선택한
		//항목을 콘솔에 출력
		...
		btnInsert.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
			 ...
			 //name은 좌우 공백 없이 3자 이상
			 if(name.trim().length() < 3) {
				 JOptionPane.showMessageDialog(null, "이름은 3자 이상 필수 입력");
				 return;
			 }
			//pgone은 필수 입력
		...
		btnDelete.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				//선택이 안된 경우는 음수
				if(idx < 0)	{
					JOptionPane.showMessageDialog(null, "선택을하고 삭제를 누르세요");
					 return;
				}
	...
	//배치되는 컴포넌트크기들을 가지고 프레임의 사이즈를 조정
	pack();
